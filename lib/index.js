...
function handleRouteApiRequest(req, res) {
	...
	if(req.params.routeid == 1) {
		route = getRouteFromList(1);
	} else if(req.params.routeid == 2) {
		route = getRouteFromList(2);
	}
...
function getTrafficForRouteList(vRouteList) {
	// Rufe bestimmte Routen von der API ab.
	if(vRouteList) {
		vRouteList.forEach(function(route, index) {
		...
		console.log('Keine Routen übergeben, die abgerufen werden können.');
	}
}
...
function createCustomCronJob(customCronJob) {
	...
	var job = new CronJob(customCronJob.cronTime, function() {
		...
		var cronRouteList = [];
		for (var i = 0; i < customCronJob.routeList.length; i++) {
			var route = getRouteFromList(customCronJob.routeList[i]);
			cronRouteList.push(route);
		}
		console.log('[' + cronRouteList.length + '] Routen für Cronjobs[' + customCronJob.description + '] gefunden. Übergebe Liste zum abruf.');
		getTrafficForRouteList(cronRouteList);
